...
export class EmailService {
  ...
  static async checkTablesExist(): Promise<boolean> {
    try {
      ...
      if (error && error.code === '42P01') {
        console.error('Les tables d\'email n\'existent pas. Veuillez exécuter le script SQL pour les créer.');
        return false;
      }
  ...
  static async processEmailQueue(maxEmails: number = 10): Promise<number> {
    try {
      ...
      if (!tablesExist) {
        console.log('Les tables d\'email n\'existent pas. Le traitement des emails est désactivé.');
        return 0;
      }
